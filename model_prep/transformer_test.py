...
def model_transformer_test(train_data, test_data):
    import pandas as pd
    import numpy as np
    ...
    sv_trainX = train_data.loc[train_data['subvillage'].str.len()>3, ['latitude', 'longitude']]
    sv_trainy = train_data.loc[train_data['subvillage'].str.len()>3, ['subvillage']]
    ...
    gps_trainX = train_data.loc[train_data['gps_height']>0, ['latitude', 'longitude']]
    gps_trainy = train_data.loc[train_data['gps_height']>0, ['gps_height']]
    ...
    data.drop(columns=['id','source_type', 'source_class', 'extraction_type_group', 'extraction_type_class', 
